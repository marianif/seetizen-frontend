export const PLACES = [
  {
    id: "1",
    image_url: "/public/houses_images/casa-museo-zona-rosato.jpeg",
    house_name: "Casa Museo Zona Rosato",
    address: "Via Exilles, 84",
    coords: {
      lat: 45.0815745,
      lng: 7.6325702,
    },
    program: {
      events: [
        {
          exhibitions: [
            {
              day: "15 Aprile",
              hours: "16-21",
              theme:
                "Simona Galeotti/Turi Rapisarda: 'Storie di Spettri e Fantasmi' ",
              description:
                "Le foto raccontano di luoghi in cui coesistono molteplici dimensioni. I “fantasmi” che infestano la realtà sono evocati attraverso figure evanescenti inserite nell’ambiente.Siamo dentro un’esperienza carica di visioni, storie da raccontare, avventure fantastiche, spettri e fantasmi. Non ci sono manipolazioni al computer o in camera oscura, le immagini vengono stampate con un metodo tradizionale ai sali d’argento. Simona Galeotti e Turi Rapisarda nasce vivono a Torino.  www.simonagaleotti.it (Giova)Le foto raccontano di luoghi “ricchi di dimensioni” – al suono popolare – “ricchi di fantasmi”- che ricerchiamo attraverso dei “richiami” ovvero disegni figure trasparenti inserite nell’ambiente. Siamo dentro un’esperienza religioso-spirituale, magico-scientifica. Quando mostriamo le nostre foto di spettri e di fantasmi in pubblico, accendiamo un faccia a faccia alla pari carico di visioni, di storie da raccontare, ognuno ha la sua, ognuno ha vissuto esperienze irregolari drammatiche di altre vite, avventure fantastiche, di spettri, di fantasmi.SIMONA GALEOTTI nata a Torino dove vive lavora.www.simonagaleotti.it TURI RAPISARDA è nato a Catania, vive lavora tra Piemonte e Sicilia",
            },
          ],
        },
        {
          dayTwo: [
            {
              hours: "16-22",
              theme: "Esposizione Turi e Galeotti",
            },
          ],
        },
      ],
    },
  },
  {
    id: "2",
    image_url: "/public/houses_images/apt-gallery.jpeg",
    house_name: "Casa Bosendofer",
    address: "Via Milano, 20",
    coords: {
      lat: 45.075183,
      lng: 7.6826686,
    },
    program: {
      events: [
        {
          dayOne: [
            {
              hours: "16-17",
              theme: "Esposizione Gallace",
            },
            {
              hours: "17-18",
              theme: "Esposizione Gallace + Piano Solo",
            },
            {
              hours: "17-18",
              theme: "Esposizione Gallace + Piano Solo",
            },
          ],
        },
        {
          dayTwo: [
            {
              hours: "16-22",
              theme: "Esposizione Turi e Galeotti",
            },
          ],
        },
      ],
      // events: [
      //   {
      //     hours: "16-17",
      //     theme: "Esposizione",
      //   },
      //   {
      //     hours: "17-18",
      //     theme: "Piano + lettura poesia",
      //   },
      //   {
      //     hours: "18-19",
      //     theme: "Esposizione",
      //   },
      //   {
      //     hours: "19-20",
      //     theme: "Esposizione Piano Solo",
      //   },
      //   {
      //     hours: "20-21",
      //     theme: "Esposizione ",
      //   },
      //   {
      //     hours: "21-22",
      //     theme: "Esposizione ",
      //   },
      // ],
    },
  },
  {
    id: "3",
    image_url: "/public/houses_images/casa-moringa.jpeg",
    house_name: "Casa Moringa",
    address: "Via Borgo Dora, 22",
    coords: {
      lat: 45.0795914,
      lng: 7.6838146,
    },
    program: {
      events: [
        {
          hours: "16-17",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "17-18",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "18-19",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere), performance Verticalista",
        },
        {
          hours: "19-20",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "20-21",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere), Jam di contact dance",
        },
        {
          hours: "21-22",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere), Jam di contact dance",
        },
      ],
    },
  },
  {
    id: "4",
    image_url: "/public/houses_images/apt-gallery.jpeg",
    house_name: "Via Bava, 19",
    address: "Via Bava, 19",
    coords: {
      lat: 45.0672302,
      lng: 7.6993797,
    },
    program: {
      events: [
        {
          hours: "16-17",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "17-18",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "18-19",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "19-20",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "20-21",
          theme:
            "Esposizione Marco + Video installazione Terry, Live Saturno Contro + Visual",
        },
        {
          hours: "21-22",
          theme: "Esposizione Marco + Video installazione Terry",
        },
      ],
    },
  },
  {
    id: "5",
    image_url: "/public/houses_images/apt-gallery.jpeg",
    house_name: "Apartment Gallery Rooftop",
    address: "Corso San Maurizio, 61",
    coords: {
      lat: 45.0684126,
      lng: 7.6962544,
    },
    program: {
      events: [
        {
          hours: "16-17",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "17-18",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "18-19",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "19-20",
          theme: "Esposizione Marco + Video installazione Terry",
        },
        {
          hours: "20-21",
          theme:
            "Esposizione Marco + Video installazione Terry, Live Saturno Contro + Visual",
        },
        {
          hours: "21-22",
          theme: "Esposizione Marco + Video installazione Terry",
        },
      ],
    },
  },
  {
    id: "6",
    image_url: "/public/houses_images/apt-gallery.jpeg",
    house_name: "Casa Mercanti",
    address: "Via San Camillo de Lellis, 30",
    coords: {
      lat: 45.069793,
      lng: 7.6797288,
    },
    program: {
      events: [
        {
          hours: "16-17",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "17-18",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "18-19",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "19-20",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "20-21",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "21-22",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
      ],
    },
  },
  {
    id: "7",
    image_url: "/public/houses_images/apt-gallery.jpeg",
    house_name: "Casa Yaga",
    address: "Via Giacinto Carena, 10",
    coords: {
      lat: 45.0790856,
      lng: 7.6676527,
    },
    program: {
      events: [
        {
          hours: "16-17",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "17-18",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "18-19",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "19-20",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "20-21",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
        {
          hours: "21-22",
          theme:
            "Esposizione Federica Moi (illustratrice) e Mattia Maio (disegni macchina da scrivere)",
        },
      ],
    },
  },
];

export const PROGRAM = {
  houses: [
    {
      id: "7",
      houseName: "Casa Museo Zona Rosato",
      address: "Via Exilles 84",
      coords: {
        lat: 45.0815745,
        lng: 7.6325702,
      },
      description:
        "A Torino tra le case protagoniste dell’itinerario artistico di Àprile c’è Casa Museo Zona Rosato. Una casa-laboratorio, a pian terreno, che una volta al mese dà spazio agli artisti con mostre e perfomance.  Dietro il progetto c’è lo scultore avellinese Gerardo Rosato che oltre a espandersi in cortile e nella hall dell’hotel a fianco, in occasione del Festival presenta un nuovo ambiente: l’Agnelleria",
    },
    {
      id: "1",
      houseName: "Via Bava 19",
      address: "Via Bava, 19",
      coords: {
        lat: 45.0672302,
        lng: 7.6993797,
      },
      description:
        "Benvenuti in ViaBava19, due vani e cucina di libera espressione. Dal 2021, una volta al mese, gli spazi di questo tipico “appartamento” di Vanchiglia si aprono a dipinti, fotografie, performance e a tanta bella gente. All’inizio la motivazione era creare una rete artistica e creativa alternativa. Poi è nato Àprile che è il l’esatta manifestazione di quella rete anelata di cui fanno parte molti artisti incontrati quasi per caso anzi per casa",
    },
    {
      id: "3",
      houseName: "Casa Moringa",
      address: "Via Borgo Dora, 22",
      coords: {
        lat: 45.0795914,
        lng: 7.6838146,
      },
      description:
        "Casa Moringa  domina la piazza del Balòn, lo storico mercato dell’antiquariato di Torino. Tutti i sabati e una domenica al mese i vicoli di Borgo Dora straripano di colori e suoni da tutto il mondo facendone un crocevia di culture. La casa ha lo spirito di questo luogo e la personalità della sua incredibile masnadiera: l’acrobata, ballerina, performer @manueilartist ! Il 15 e 16 aprile la casa accoglierà artisti dalle molteplici tecniche espressive. In alcuni momenti gli spettatori saranno invitati a interagire con musicisti, artisti visivi e performer, divenendo essi stessi parte integrante dell'atto artistico. Il movimento è al servizio della scena!",
    },
    {
      id: "5",
      houseName: "Apartment Gallery Rooftop",
      address: "Corso San Maurizio, 61",
      coords: {
        lat: 45.0684126,
        lng: 7.6962544,
      },
      description:
        "Tra gli appartamenti presenti al Festival Àprile c’è anche Apartment Gallery, nella versione Rooftop Experience. I curatori Neri Muccini e Sonja Krstić festeggiano 3 anni della loro attività di curatela in gallerie d’appartamento proponendo nuovi artisti con installazioni site specific, performance e talk. La cornice sarà uno spazioso e suggestivo rooftop nel cuore del quartiere Vanchiglia.",
    },
    {
      id: "2",
      houseName: "Piano B",
      address: "Via Milano, 20",
      coords: {
        lat: 45.075183,
        lng: 7.6826686,
      },
      description:
        "Percorrendo Via Milano fino alle soglie di Piazza della Repubblica ci si trova nel cuore di un quartiere connesso col mondo intero. Filippo Juvarra, architetto di capolavori quali la Basilica di Superga e la Palazzina di Caccia di Stupinigi ha qui progettato a inizio Settecento una serie di palazzi con maschere di tori, leoni e grifoni: uno di questi è diventato un co-housing capace di promuovere un modo di abitare innovativo fondato sulla qualità delle relazioni di vicinato e sulla condivisione di sorrisi, chiacchiere e sedie per gli ospiti. L’appartamento che si affaccia sul tetto della Galleria Umberto I scoppia di musica: è proprio il Piano B. Esso custodisce al suo interno un Bösendorfer Grand Piano, più unico che raro. Il concerto non è ancora iniziato ma c’è già una lunga coda!",
    },
    {
      id: "4",
      houseName: "Casa Mercanti",
      address: "Via San Camillo de Lellis, 30",
      coords: {
        lat: 45.069793,
        lng: 7.6797288,
      },
      description:
        "Incastonata tra Piazza Solferino e Piazza Castello, la Casa dei Mercanti svetta sopra il vortice delle misteriose energie che animano la città di Torino e sembra essere essa stessa un luogo mistico, che illude, confonde e incanta coloro che vi si avventurano. Come un sacro monastero, un castello stregato o un labirinto, questo luogo sottopone i suoi visitatori a esperienze che mettono alla prova lo spirito e il corpo: la prima consiste in una lunga serie di gradini (sei piani!), superati i quali si giunge all’agognata soglia. Qui, tra nicchie, pareti curve e finestre che sbirciano, improvvisi varchi di luce sembrano aprirsi lasciando intravedere i molti ambienti dell’appartamento. Ballatoi, lanterne e passaggi segreti si sviluppano in lungo e in largo, in alto e in basso, e mentre oscuri coni d’ombra s’allungano sui muti oggetti, l’aria non è ferma e nella casa succedono molte cose:",
    },
    {
      id: "6",
      houseName: "Casa Carena",
      address: "Via Giacinto Carena, 10",
      coords: {
        lat: 45.0790856,
        lng: 7.6676527,
      },
      description:
        "A pochi passi dalla sede del collettivo artistico “Mostro”, nel quartiere San Donato, c’è una casa che è tutta una festa: è la casa di Yaga. Vorreste sapere qualcosina di più su questo luogo misterioso? Avete un solo modo: andarla a scoprire! ",
    },
  ],
  exhibitions: [
    {
      houseId: "7",
      houseName: "Casa Museo Zona Rosato",
      address: "Via Exilles 84",
      title: "Storie di Spettri e Fantasmi",
      description:
        "Le foto di Simona Galeotti e Turi Rapisarda raccontano di luoghi in cui coesistono molteplici dimensioni. “Fantasmi” che infestano la realtà evocati attraverso figure evanescenti. Siamo dentro un’esperienza carica di visioni, spettri, e storie da raccontare. Con la collaborazione di Oolalala Gallery, progetto artistico nomade.",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "1",
      houseName: "Via Bava 19",
      address: "Via Bava 19",
      title: "Esposizione: 'Camera blu' di Marco Gagliardi",
      description:
        " Camera blu è il progetto che Marco Gagliardi prepara su misura per ViaBava19. La tecnica della cianotipia apre a una narrazione intima sospesa: fotogrammi interiori come singole percezioni che scorrono in un fiume, la cui acqua è pellicola dell'immaginario. Marco Gagliardi, classe 1996, si è dedicato in maniera continuativa alla ricerca di una libera espressione e di un linguaggio artistico personale, utilizzando vari media e tecniche sperimentali. Nel suo lavoro sono centrali gli elementi e le immagini della natura; i richiami al tempo/spazio; l’indagine su luci e ombre, tutto relazionato al corpo e all’essere umano. Come un alchimista che tratta la materia per scoprire i segreti e le potenziali trasformazioni in essa racchiuse ognuno di noi, alla luce del sole, si allinea con la propria ombra, riflesso mutabile di storie che non conosciamo",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "1",
      houseName: "Via Bava 19",
      address: "Via Bava 19",
      title: "Videoinstallazione: Oedipus Rex",
      description:
        "OEDIPUS REX nasce da Teresa Verrascina e Matilde Panetta in collaborazione con Katia Licenziato. Il cortometraggio è la narrazione dell’ Edipo re di Sofocle, riscritto in flash visivi, tra buio e visioni accecanti. Essere ciechi rispetto a se stessi, non guardare dentro, non essere consapevoli della propria carne e del proprio destino. “Nessun essere mortale possiede arte profetica”. Il corto è stato girato esattamente nel posto in cui lo spettatore potrà esperirlo, nel teatrino di ViaBava19. Scritto e diretto da Teresa Verrascina, Matilde Panetta con la collaborazione di Katia Licenziato. Musiche e produzione audio di Claudio Cardia.",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "3",
      houseName: "Casa Moringa",
      address: "Via Borgo Dora, 22",
      title: "Esposizione: “0, 2, 4, 8, 1000 ZAMPE” di  Federica Moi",
      description:
        "Federica Moi, classe 1997, apprende presso l’Accademia Albertina di Belle Arti di Torino  tecniche di pittura, disegno tradizionale, illustrazione digitale. Una passione per le tradizioni nel mondo e i miti del passato ispira le sue figure stilizzate dai colori acidi: simboli, metafore e piccole finestre sul suo immaginario.0, 2, 4, 8, 1000 ZAMPE è una serie di illustrazioni realizzate con matite colorate, grafite e china raffiguranti alcuni tra i più noti mostri e animali mitologici ispirati al “Manuale di zoologia fantastica” di Jorge Luis Borges e Margarita Guerrero.",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "3",
      houseName: "Casa Moringa",
      address: "Via Borgo Dora, 22",
      title: "Esposizione: “Taping and typing” di Mattia Maio",
      description:
        "Mattia Maio in arte Matolivetti, classe 1989, intraprende un percorso artistico alla ricerca di mezzi espressivi non convenzionali: tratti distintivi della sua produzione sono macchina da scrivere e scotch da pacchi. Nella performance “Taping and typing” realizza disegni con la macchina da scrivere imprimendo sul foglio le lettere in chiaroscuro. Le opere con il nastro adesivo sono invece il  risultato di vari strati di scotch sagomati con un bisturi e retroilluminati",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "6",
      houseName: "Casa Carena",
      address: "Via Giacinto Carena, 10",
      title:
        "Jam murales in casa aperto al pubblico con Yaga, Ghitka, Jacopino, Lisa, Elisa e Milena",
      description:
        "Yaga, Ghitka, Jacopino, Lisa, Elisa e Milena sono artisti che lavorano a torino negli ambienti informali. Si esibiranno dipingendo sui muri di Casa Carena in un murales collaborativo aperto al pubblico. Il pubblico potrà partecipare, e i colori saranno messi a disposizione da Yaga. Altre opere degli artisti saranno esposte in vari angoli della casa: librerie, scaffali.",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "5",
      houseName: "Apartment Gallery Rooftop",
      address: "Corso San Maurizio, 61",
      title:
        "Installazione multimediale di Tteus Chiba, Hyperspeed Hallucinations e MoodooM ",
      description:
        "Durante il Festival, Tteus Chiba, Hyperspeed Hallucinations e MoodooM presenteranno le proprie opere integrandole in un'installazione aperta all’interazione dei presenti, collaborativa  e in divenire, capace di trasfigurare le opere degli artisti, le proprie percezioni e i propri volti",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "5",
      houseName: "Apartment Gallery Rooftop",
      address: "Corso San Maurizio, 61",
      title: "Esposizione delle opere di Contatto Meccano",
      description:
        "Marco Primavera (in arte Contatto Meccano) è un artista poliedrico classe 1984 che, trasferitosi a Torino per motivi di studio, vi intraprende un lungo percorso di crescita artistica segnato dall'incontro con la cultura underground. É evidente nei suoi lavori l'influenza delle avanguardie. Il ritrovamento di una Lettera 22 Olivetti abbandonata e di un manichino portano alle prime creazioni Ready Made sotto forma di sculture. Le opere di Contatto Meccano prendono forma e vita propria, infuse della passione creativa che le porta alla luce, materializzandosi libere da ogni costrizione disciplinare: al di là di scultura, fotografia, grafiche, arredo d'interni",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "2",
      houseName: "Piano B",
      address: "Via Milano, 20",
      title: "“UNFAMILIAR” di Giuseppe Gallace",
      description:
        "Il protagonista della pittura di @giuseppe_wallas_gallace sembra essere ciò che non è presente: l’artista sembra voler alludere a qualcosa di mancante, dare forma a quel che forma non ha. Una pittura corposa caratterizzata da una forte matericità racconta ora figure equivoche e indecifrabili ora la dimensione intima dei paesaggi domestici.  Ciò che dovrebbe essere familiare diviene improvvisamente anomalo. Giuseppe Gallace, classe 1993, vive e opera a Torino dal 2016. Cultore della Materia in Pittura, ha collaborato con il MAU e la Pinacoteca Albertina e affianca all’attività di artista quella di insegnante di pittura e disegno.",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "4",
      houseName: "Casa Mercanti",
      address: "Via Camillo de Lellis, 30",
      title: "“Trittico della città” + “Inside Turin” di GIBRAH",
      description:
        "La ricerca del pittore Giuliano Brancale, in arte GIBRAH, da tempo si rivolge alla rappresentazione  degli spazi urbani e degli interni domestici: essi  non sono altro  che la realtà interna, nascosta allo sguardo, dei palazzi e dei condomini.La sua ricerca è iniziata con il Trittico della Città, dipinto interamente dal vero da un appartamento al settimo piano in Barriera di Milano.Oltre al Trittico, esporrà due tele prodotte per l’occasione di due corti interne di altrettante case coinvolte nel festival",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "4",
      houseName: "Casa Mercanti",
      address: "Via Camillo de Lellis, 30",
      title: "Esposizione: “Luna Rossa” di HIDE",
      description:
        "Tommaso Pigliapoco in arte HIDE, laureando in Pittura all’Accademia Albertina di Belle Arti, fa parte dell’associazione culturale Teatro Carillon con la quale partecipa alla realizzazione di spettacoli  performativi di videomapping. Luna Rossa è un progetto di sei dipinti che raccontano, attraverso il viaggio iniziatico, il rapporto tra l’uomo e il simbolo dell’eclissi lunare nelle tappe fondamentali del percorso umano",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
    {
      houseId: "4",
      houseName: "Casa Mercanti",
      address: "Via Camillo de Lellis, 30",
      title: "Esposizione fotografica di Anonima Areste",
      description:
        "La stanza privata è il luogo dove posso vivere il corpo senzagiudizio. In questa stanza mi rifletto, narcisa, osservo ilmio corpo e lo rivedo da una prospettiva sempre diversa,sentendomi in colpa nei momenti in cui lo amo, non sentendomimai in colpa nei momenti in cui lo odio. Primo nemico, ultimaarma, condanna, corazza. Corpo, sempre in cambiamento, maipiù uguale a prima. Nella stanza privata il corpo è una portad’accesso verso l’altro corpo. È il corpo sensuale ederotico, che coglie una verità intima non definibileverbalmente. È il corpo nella sua sincerità spaventosa. Corpoche ama, corpo che mostra, corpo che semplicemente è.",
      timeRange: "16 - 21",
      artistIds: ["", ""],
    },
  ],
  performances: {
    dayOne: [
      {
        houseId: "5",
        houseName: "Apartment Gallery Rooftop",
        address: "Corso S. Maurizio, 61",
        title:
          "Presentazione Tesi “La Galleria d’appartamento: un percorso verso nuovi spazi d’arte”",
        description:
          "APARTMENT GALLERY Neri Muccini & Sonja Krstić Apartment Gallery nasce nel 2020 da un’idea di Neri Muccini, che, utilizzando gli ambienti del suo appartamento come spazi laboratoriali ed espositivi, decide di organizzare una mostra dei propri lavori. L'appartamento diventa quindi uno spazio espositivo del tutto unico, dove le persone si incontrano e si confrontano  con le opere in esposizione, calandosi in un’atmosfera inclusiva ed intima. Dopo un’intensa fase espositiva, i curatori decidono di iniziare a fare ricerca per conoscere meglio il fenomeno delle gallerie d’appartamento. Neri e Sonja scrivono una tesi collaborativa dal titolo “La galleria  d’appartamento: un percorso verso nuovi spazi d’arte. Interviste e testimonianze sull’esporre”. I risultati  del lavoro sono entusiasmanti e nel giro di pochi mesi la ricerca inizia a circolare, interessando sempre più  persone",
        timeRange: "16",
        artistIds: ["", ""],
      },
      {
        houseId: "5",
        houseName: "Apartment Gallery Rooftop",
        address: "Corso S. Maurizio, 61",
        title: "Performance installazione multimediale di Manueila",
        description:
          "Manuela Pilloni, in arte Manueila, è una performer alla costante ricerca della genuinità del gesto corporeo. Si diploma al Teatro Fisico nel 2013 e continua la sua formazione artistica su verticalismo, contorsionismo, clown e teatro danza.In occasione del Festival interagirà con l’installazione multimediale di Tteus Chiba, Hyperspeed Hallucinations e MoodooM, diventandone parte integrante. Rappresenterà l'elemento umano, reale, che si intreccia con il digitale e l'analogico",
        timeRange: "19",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title: "Performance Verticalista di Manueila",
        description:
          "Manuela Pilloni, in arte Manueila, è una performer alla costante ricerca della genuinità del gesto corporeo. Si diploma al Teatro Fisico nel 2013 e continua la sua formazione artistica su verticalismo, contorsionismo, clown e teatro danza.In occasione del Festival interagirà con l’installazione multimediale di Tteus Chiba, Hyperspeed Hallucinations e MoodooM, diventandone parte integrante. Rappresenterà l'elemento umano, reale, che si intreccia con il digitale e l'analogico",
        timeRange: "17.10 - 17.30",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title: "“FantastiK!” di Manuela Pilloni musicata da Jay Happy",
        description:
          "FantastiK! è una performance di Manuela Pilloni musicata da Jay Happy. Il pubblico è chiamato a scegliere dal mazzo una carta raffigurante un essere fantastico e la performer, sulle note della band, lo ricreerà seduta stante. Il risultato è una musica in costante trasformazione, eclettica, che preferisce creare momenti volatili piuttosto che oggetti statici.Manuela Pilloni, in arte @manueila, è una performer alla costante ricerca della genuinità del gesto corporeo. Si diploma al Teatro Fisico nel 2013 e continua la sua formazione artistica su verticalismo, contorsionismo, clown e teatro danza.Jay Happy è il progetto live del collettivo internazionale donezk collective. I brani del collettivo vengono arrangiati e trasfigurati dagli interpreti di turno, ma anche dalla strumentazione e dal contesto",
        timeRange: "18 - 18.30",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title: "Concerto Live di Jay Happy",
        description:
          "JAY HAPPY non è sempre la stessa persona, ne sempre lestesse persone. Attualmente si lascia interpretare da Julian Happe,polistrumentista cantante e scribacchino tedesco (se gli chiedete di dovevi risponderà ”..mmm da qualche parte nel centro”) e Pietro Pagliana,musicista, cantautore e produttore italico, tristemente piemontese. Ilgruppo ha viaggiato e vissuto in Germania, Francia, Portogallo e altriposti, suonando e cantando per strada, in molteplici bar, teatri, qualchesala concerti, studi e case di amici e conoscenti e spesso anche inparchi e foreste",
        timeRange: "19.00 - 20.00",
        artistIds: ["", ""],
      },
      {
        houseId: "2",
        houseName: "Piano B",
        address: "Via Milano, 20",
        title: "Piano solo di Ludovico Bellucci",
        description:
          "@ludovico_bellucci ha 18 anni e suona il pianoforte dall'età di 8. Frequenta il Liceo Musicale Cavour di Torino e si è sempre dedicato alla composizione seguendo un personale estro, ma negli ultimi due anni compone colonne sonore. Ha eseguito dal vivo la colonna sonora da lui ideata per i film Paris qui dort (1925, René Clair) e La caduta di Troia (1911, Giovanni Pastrone). Ha composto la colonna sonora originale del cortometraggio Dura Lex Sed Lex, che si è aggiudicato il premio finale al “believe film festival” di Verona 2022. Si è già esibito al Conservatorio di Torino e alla Cineteca di Karlsrühe (Germania). Ludovico Bellucci è presente su tutti i Digital Store e il suo repertorio include altri brani originali che contraddistinguono il suo stile come Renaissance e Senza Tempo. Quando parla della sua musica la definisce trasognante e retrò, legata al periodo della Belle Èpoque ma non scevra di suggestioni romantiche",
        timeRange: "18.00 e 19.30",
        artistIds: ["", ""],
      },
      {
        houseId: "4",
        houseName: "Casa Mercanti",
        address: "Via Camillo de Lellis, 30",
        title:
          "Videomapping + performance musicale: “BASA (Barriera Space Agency)” di Teatro Carillon",
        description:
          "Teatro Carillon è una compagnia teatrale e colettivo interdisciplinare nato nel 1983 a Torino, nel quartiere popolare di Barriera di Milano. Dopo decenni dedicati allo sviluppo di linguaggi sperimentali in ambito prettamente teatrale, dal 2015 la compagnia  si è aperta a nuove realtà. In BASA (Barriera Space Agency), dj virtuali accompagnano il tappeto mirabolante di videomapping site specific. Partendo dalla stratificazione di lavori proiettati si genera spontaneamente il dibattito della virtualità nell’arte, metafora del distacco emotivo osservatore-opera. Si tratta di una riflessione sulla perdita di profondità comunicativa ma anche di interazione contaminativa e definizioni di reti interterritoriali",
        timeRange: "16 - 21",
        artistIds: ["", ""],
      },
      {
        houseId: "4",
        houseName: "Casa Mercanti",
        address: "Via Camillo de Lellis, 30",
        title: "Spettacolo Teatrale: HAPPY LIFE: Estratti di vite a caso”",
        description:
          "Un letto. Due attrici, una tournée. Dora e Zoe si svegliano... Ma perché non dormono alle due di notte? Il pubblico ascolta. È invitato? Ovvio che no! Cioè, ti hanno mai invitato in una camera con due persone alle due di notte? Non credo proprio. Ti interessa ascoltare cosa si dicono due sconosciute? Chi è Francesco Da Torre? Sicuro di volerlo scoprire? E cosa c’entra il teatro? Con un linguaggio comico-grottesco e talvolta con note di assurda follia Dora e Zoe apriranno la porta della loro camera d’hotel, mostrandosi così come sono. La compagnia Malament’Espress (MALES) nasce a Firenze nel 2017 da un’idea di Rebecca Benedettini e Valentina Brancale. Scrive, dirige e interpreta propri testi inediti. Esordisce con “Monolo-chi? Monolo-che? Estratti di vite a caso” segnando l’impronta comico-grottesca, tratto identificativo della compagnia. Nel 2019 entra nella compagnia Sofia Busia con “ Waiting for ” una commedia brillante sul tema della corruzione, in collaborazione con l’associazione “Libera contro le mafie” di Sesto Fiorentino (FI); nel 2021 con “L’Ascensore”, un thriller psicologico, la compagnia si esibisce in scena al Teatro dell’Affratellamento (FI) e apre una collaborazione con attori e maestranze tecniche esterne. Nel Settembre 2022 debutta al’Ortyx Drama Festival di Siracusa con “Happy Life, Estratti di vite a caso” una rivisitazione del primo spettacolo.",
        timeRange: "20",
        artistIds: ["", ""],
      },
      {
        houseId: "1",
        houseName: "Via Bava 19",
        address: "Via Bava 19",
        title: "Live Audiovisivo di Chinò e il mare",
        description:
          "In Chinòa s’intersecano più generi, più linguaggi. Un live audiovisivo e autogenerativo, oltre che a essere inedito, dove la musica elettronica si compone di canzoni smembrate prima ancora di essere inedite. Le immagini in movimento mostrano una realtà che si infrange sulle onde sonore attraverso una connessione digitale: un tentativo di espressione sinestetica. Linguaggi, pensati da sempre come diversi, si con-fondono tra loro per raccontare il replicarsi di una presenza ogni volta diversa. Il progetto nasce nel 2022 da un esperimento IDM di Chinòa e il mare, progetto di cantautorato in ambienti elettronici da Bologna",
        timeRange: "17",
        artistIds: ["", ""],
      },
      {
        houseId: "1",
        houseName: "Via Bava 19",
        address: "Via Bava 19",
        title: "Live Concerto di Saturno Contro",
        description:
          "Saturno Contro è un duo di musica di genere electro/glitch/ambient che nasce a Bologna nel 2020: nella loro musica sperimentale suoni ambient, electro e i.d.m. si fondono in un'ondata di pura avant-garde.Il loro primo 'Saturno Contro EP' (2021), feat. Malika Baamarouf, segna il loro incontro col pop. Per il Festival metteranno in scena una performance audiovisiva con la collaborazione del visual artist Jacopo Sandrin. La performance racconta l’incontro tra paesaggi surburbani e natura in una società veloce, dove gli estremi del progresso meccanico incontrano l’umanità carnale che alterna momenti prettamente musicali a momenti letterari",
        timeRange: "19.30",
        artistIds: ["", ""],
      },
    ],
    dayTwo: [
      {
        houseId: "5",
        houseName: "Apartment Gallery Rooftop",
        address: "Corso S. Maurizio, 61",
        title: "Tesi : Edilizia pubblica a Torino dal secondo dopoguerra ",
        description:
          "La costruzione del paesaggio periferico torinese è stata caratterizzata dalla domanda abitativa conseguente allo sviluppo industriale che portò alla realizzazione di edilizia residenziale pubblica. Tali edificazioni, localizzate ai margini e separate sia dalla città esistente che dalle nuove periferie, offrì ad architetti, urbanisti e ingegneri un campo di sperimentazione progettuale su idee di città, tipi e misure degli spazi abitabili, tecniche costruttive e materiali. La tesi vuole far riflettere sul ruolo che ha avuto l’edilizia pubblica nella costruzione della città e sul suo significato oggi. L’autrice Giulia Pechino, laureata in Beni Culturali e specializzata in comunicazione e valorizzazione del patrimonio artistico ci racconta l’evoluzione delle morfologie architettoniche attraverso le sue esplorazioni urbane e bibliografiche",
        timeRange: "16",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title: "Performance Verticalista di Manueila",
        description:
          "Manuela Pilloni, in arte Manueila, è una performer alla costante ricerca della genuinità del gesto corporeo. Si diploma al Teatro Fisico nel 2013 e continua la sua formazione artistica su verticalismo, contorsionismo, clown e teatro danza.In occasione del Festival interagirà con l’installazione multimediale di Tteus Chiba, Hyperspeed Hallucinations e MoodooM, diventandone parte integrante. Rappresenterà l'elemento umano, reale, che si intreccia con il digitale e l'analogico",
        timeRange: "16.30",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title: "“FantastiK!” di Manuela Pilloni musicata da Jay Happy",
        description:
          "FantastiK! è una performance di Manuela Pilloni musicata da Jay Happy. Il pubblico è chiamato a scegliere dal mazzo una carta raffigurante un essere fantastico e la performer, sulle note della band, lo ricreerà seduta stante. Il risultato è una musica in costante trasformazione, eclettica, che preferisce creare momenti volatili piuttosto che oggetti statici.Manuela Pilloni, in arte @manueila, è una performer alla costante ricerca della genuinità del gesto corporeo. Si diploma al Teatro Fisico nel 2013 e continua la sua formazione artistica su verticalismo, contorsionismo, clown e teatro danza.Jay Happy è il progetto live del collettivo internazionale donezk collective. I brani del collettivo vengono arrangiati e trasfigurati dagli interpreti di turno, ma anche dalla strumentazione e dal contesto",
        timeRange: "18 - 19",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title:
          "Performance disegno dal vivo: “Taping and typing” di Mattia Maio",
        description:
          "Mattia Maio in arte Matolivetti, classe 1989, intraprende un percorso artistico alla ricerca di mezzi espressivi non convenzionali: tratti distintivi della sua produzione sono macchina da scrivere e scotch da pacchi. Nella performance “Taping and typing” realizza disegni con la macchina da scrivere imprimendo sul foglio le lettere in chiaroscuro. Le opere con il nastro adesivo sono invece il  risultato di vari strati di scotch sagomati con un bisturi e retroilluminati",
        timeRange: "19.00 - 19.15",
        artistIds: ["", ""],
      },
      {
        houseId: "3",
        houseName: "Casa Moringa",
        address: "Via Borgo Dora, 22",
        title:
          "Jam di Contact Dance eseguita da Cristina da Ponte e musicata da Aire Duo ",
        description:
          "La jam di contact dance eseguita da Cristina da Ponte e musicata da @Aire_Duo è aperta a chiunque voglia sperimentare. Si tratta di una danza nata negli anni 70 in America in risposta ai dogmatismi e le regole della danza moderna. Oggi praticata in quasi tutto il mondo permette di entrare in connessione attraverso il movimento.  Aire_Duo nasce dall’incontro di Massimo di Pierro e Alessandra Ferraris. Chitarra, loop station ed effetti si fondono ai ritmi dell’hangdrum per arrangiamenti sonori sempre nuovi che coinvolgono.Cristina Da Ponte pone la poesia al centro della sua ricerca. Lo studio del movimento comincia da molto giovane con la ginnastica artistica, prosegue poi nell’ambito del teatro e della danza contemporanea. Da sempre è autrice e interprete dei suoi lavori, creatrice di performance site-specific di danza in urbana e in natura in collaborazione con diversi artisti. Attualmente lavora con il gruppo Shared Training Torino e fa parte del collettivo C.I.F.R.A._danzateatro",
        timeRange: "19.40 - 21",
        artistIds: ["", ""],
      },
      {
        houseId: "2",
        houseName: "Piano B",
        address: "Via Milano, 20",
        title: "Piano solo di Ludovico Bellucci",
        description:
          "Ludovico Bellucci ha 18 anni e suona il pianoforte dall’età di 8. Frequenta il Liceo Musicale Cavour di Torino e si è sempre dedicato alla composizione seguendo un personale estro, ma negli ultimi due anni compone colonne sonore. Si è già esibito al Conservatorio di Torino e alla Cineteca di Karlsrühe con sonorizzazioni dal vivo dei film “Paris qui dort” (1925) e “La caduta di Troia” (1911). Ha composto la colonna sonora originale del cortometraggio “Dura Lex Sed Lex”, premio finale al “believe film festival” di Verona 2022. È presente su tutti i Digital Store e il suo repertorio include altri brani originali che contraddistinguono il suo stile come “Renaissance” e “Senza Tempo”",
        timeRange: "18.00 - 18.20 e 19.30. - 19.50",
        artistIds: ["", ""],
      },
      {
        houseId: "4",
        houseName: "Casa Mercanti",
        address: "Via Camillo de Lellis, 30",
        title:
          "Spettacolo Teatrale: HAPPY LIFE: Estratti di vite a caso” di MALES",
        description:
          "Teatro Carillon è una compagnia teatrale e colettivo interdisciplinare nato nel 1983 a Torino, nel quartiere popolare di Barriera di Milano. Dopo decenni dedicati allo sviluppo di linguaggi sperimentali in ambito prettamente teatrale, dal 2015 la compagnia  si è aperta a nuove realtà. In BASA (Barriera Space Agency), dj virtuali accompagnano il tappeto mirabolante di videomapping site specific. Partendo dalla stratificazione di lavori proiettati si genera spontaneamente il dibattito della virtualità nell’arte, metafora del distacco emotivo osservatore-opera. Si tratta di una riflessione sulla perdita di profondità comunicativa ma anche di interazione contaminativa e definizioni di reti interterritoriali",
        timeRange: "17",
        artistIds: ["", ""],
      },
      {
        houseId: "4",
        houseName: "Casa Mercanti",
        address: "Via Camillo de Lellis, 30",
        title:
          "Videomapping + performance musicale: “BASA (Barriera Space Agency)” di Teatro Carillon",
        description:
          "Teatro Carillon è una compagnia teatrale e colettivo interdisciplinare nato nel 1983 a Torino, nel quartiere popolare di Barriera di Milano. Dopo decenni dedicati allo sviluppo di linguaggi sperimentali in ambito prettamente teatrale, dal 2015 la compagnia  si è aperta a nuove realtà. In BASA (Barriera Space Agency), dj virtuali accompagnano il tappeto mirabolante di videomapping site specific. Partendo dalla stratificazione di lavori proiettati si genera spontaneamente il dibattito della virtualità nell’arte, metafora del distacco emotivo osservatore-opera. Si tratta di una riflessione sulla perdita di profondità comunicativa ma anche di interazione contaminativa e definizioni di reti interterritoriali",
        timeRange: "19 - 21",
        artistIds: ["", ""],
      },
      {
        houseId: "1",
        houseName: "Via Bava 19",
        address: "Via Bava 19",
        title: "Live Audiovisivo di Chinò e il mare",
        description:
          "In Chinòa s’intersecano più generi, più linguaggi. Un live audiovisivo e autogenerativo, oltre che a essere inedito, dove la musica elettronica si compone di canzoni smembrate prima ancora di essere inedite. Le immagini in movimento mostrano una realtà che si infrange sulle onde sonore attraverso una connessione digitale: un tentativo di espressione sinestetica. Linguaggi, pensati da sempre come diversi, si con-fondono tra loro per raccontare il replicarsi di una presenza ogni volta diversa. Il progetto nasce nel 2022 da un esperimento IDM di Chinòa e il mare, progetto di cantautorato in ambienti elettronici da Bologna",
        timeRange: "19.30",
        artistIds: ["", ""],
      },
      {
        houseId: "1",
        houseName: "Via Bava 19",
        address: "Via Bava 19",
        title: "Live Concerto di Saturno Contro",
        description:
          "Saturno Contro è un duo di musica di genere electro/glitch/ambient che nasce a Bologna nel 2020: nella loro musica sperimentale suoni ambient, electro e i.d.m. si fondono in un'ondata di pura avant-garde.Il loro primo 'Saturno Contro EP' (2021), feat. Malika Baamarouf, segna il loro incontro col pop. Per il Festival metteranno in scena una performance audiovisiva con la collaborazione del visual artist Jacopo Sandrin. La performance racconta l’incontro tra paesaggi surburbani e natura in una società veloce, dove gli estremi del progresso meccanico incontrano l’umanità carnale che alterna momenti prettamente musicali a momenti letterari",
        timeRange: "17",
        artistIds: ["", ""],
      },
    ],
  },
};

export const HOUSES = [
  {
    houseId: "1",
    name: "Via Bava 19",
  },
  {
    houseId: "2",
    name: "Piano B",
  },
  {
    houseId: "3",
    name: "Casa Moringa",
  },
  {
    houseId: "4",
    name: "Casa Mercanti",
  },
  {
    houseId: "5",
    name: "Apartement Gallery",
  },
  {
    houseId: "6",
    name: "Casa Carena",
  },
  {
    houseId: "7",
    name: "Casa Museo Zona Rosato",
  },
];
